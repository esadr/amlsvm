...
// Creates a new matrix by appending the second to the bottom of the first
// Source matrices must have same number of columns
// Does not destroy the first two matrices
// Dimensions: A - m1 x n, B - m2 x n, dest - m1+m2 x n
void CommonFuncs::combine_Mats(Mat& A, Mat& B, Mat& dest) {
    PetscInt num_cols;
    PetscInt num_row_A, num_row_B, total_num_row;
    PetscInt i, ncols;
    const PetscInt *cols;
    const PetscScalar *vals;

    MatGetSize(A, &num_row_A, &num_cols);
    MatGetSize(B, &num_row_B, NULL);

    total_num_row = num_row_A + num_row_B;

    MatCreateSeqAIJ(PETSC_COMM_SELF, total_num_row, num_cols, num_cols, NULL, &dest);

    for(i=0; i<num_row_A; i++) {
        MatGetRow(A, i, &ncols, &cols, &vals);
        MatSetValues(dest, 1, &i, num_cols, cols, vals, INSERT_VALUES);
        MatRestoreRow(A, i, &ncols, &cols, &vals);
    }
    for(i=0; i<num_row_B; i++) {
        MatGetRow(B, i, &ncols, &cols, &vals);
        PetscInt idx = i + num_row_A;
        MatSetValues(dest, 1, &idx, num_cols, cols, vals, INSERT_VALUES);
        MatRestoreRow(A, i, &ncols, &cols, &vals);
    }
