...
//                                    , std::vector<ref_results>& v_ref_results){
    ...
    // Holds solvers for concurrent thread access
    Solver a_solver[num_iter_st1 + num_iter_st2];

    unsigned int solver_id=0;

    std::vector<summary> v_summary;
    v_summary.reserve(num_iter_st1 + num_iter_st2);

    // Holds summaries for concurrent thread access
    summary a_summary[num_iter_st1 + num_iter_st2];
...
//    summary current_summary;     //map that contains all the measures
    ...
    // Since access to the ud params are read only, pointer to data is all
    // that's necessary for concurrent thread access
    ud_point * ud_params_st_1_arr = ud_params_st_1.data();

    // Setup threads
    unsigned int i;
    omp_set_num_threads(num_iter_st1);

    ETimer omp_mat_copy;

    // Copies of these Mats and Vecs are needed for each thread since PetSc
    // does not allow multiple open calls to MatGetRow()
    Mat *temp_td_p_mats, *temp_td_n_mats, *temp_vd_p_mats, *temp_vd_n_mats;
    Vec *temp_td_vol_p_vecs, *temp_td_vol_n_vecs;
    PetscMalloc1(num_iter_st1, &temp_td_p_mats);
    PetscMalloc1(num_iter_st1, &temp_td_n_mats);
    PetscMalloc1(num_iter_st1, &temp_vd_p_mats);
    PetscMalloc1(num_iter_st1, &temp_vd_n_mats);
    PetscMalloc1(num_iter_st1, &temp_td_vol_p_vecs);
    PetscMalloc1(num_iter_st1, &temp_td_vol_n_vecs);

    for(i=0; i < num_iter_st1; i++) {
        MatDuplicate(m_train_data_p, MAT_COPY_VALUES, &temp_td_p_mats[i]);
        MatDuplicate(m_train_data_n, MAT_COPY_VALUES, &temp_td_n_mats[i]);
        MatDuplicate(m_VD_p, MAT_COPY_VALUES, &temp_vd_p_mats[i]);
        MatDuplicate(m_VD_n, MAT_COPY_VALUES, &temp_vd_n_mats[i]);

        VecDuplicate(v_train_vol_p, &temp_td_vol_p_vecs[i]);
        VecCopy(v_train_vol_p, temp_td_vol_p_vecs[i]);
        VecDuplicate(v_train_vol_n, &temp_td_vol_n_vecs[i]);
        VecCopy(v_train_vol_n, temp_td_vol_n_vecs[i]);
    }

    omp_mat_copy.stop_timer("[MS][UDSepVal][OMP] Copying matrices at level", std::to_string(level));

#pragma omp parallel for default(shared) private(i)
    for(i =0; i < num_iter_st1;++i){
        summary *current_summary = &a_summary[i];
        Solver *sv = &a_solver[i];

        svm_model * curr_svm_model;

//        int rank = omp_get_thread_num();		// $Debug purposes
 ...
 */
        ...
        // Parallel code
        curr_svm_model = sv->train_model(temp_td_p_mats[i], temp_td_vol_p_vecs[i]
                                        , temp_td_n_mats[i], temp_td_vol_n_vecs[i], 1
                                        , ud_params_st_1_arr[i].C, ud_params_st_1_arr[i].G);
        //predict the validation data not the test data
        sv->predict_validation_data(temp_vd_p_mats[i], temp_vd_n_mats[i]
                                   , *current_summary
                                   , i);//solver_id);

        // Moved outside parallel code to add them in order        
//        v_solver.push_back(sv);
//        v_summary.push_back(current_summary);

//        current_summary.v_fn_idx.clear();
//        current_summary.v_fp_idx.clear();

//            sv.free_solver("[MS][UDSepVal] ");   //free the solver

/* Moved outside parallel code so summaries can be printed in order
#if dbl_MS_UDSepVal >= 3
//        paramsInst->print_summary(current_summary,"[MS][UDSepVal]", level, i, stage);
        paramsInst->print_summary(a_summary[i],"[MS][UDSepVal]", level, i, stage);		// $PARALLEL
#endif
*/
...
//        ++solver_id;
...
#if dbl_MS_UDSepVal >= 3
    for(int i=0; i<num_iter_st1; i++){
        paramsInst->print_summary(a_summary[i],"[MS][UDSepVal]", level, i, stage);
    }
#endif

    // No longer need all of the copies of the TD Mats and Vecs, just enough for stage 2
    for(i=num_iter_st2; i < num_iter_st1; i++) {
        MatDestroy(&temp_td_p_mats[i]);
        MatDestroy(&temp_td_n_mats[i]);
        MatDestroy(&temp_vd_p_mats[i]);
        MatDestroy(&temp_vd_n_mats[i]);
        VecDestroy(&temp_td_vol_p_vecs[i]);
        VecDestroy(&temp_td_vol_n_vecs[i]);
    }

    // Put solvers and summaries in vectors
    for(i=0; i<num_iter_st1;i++) {
        v_solver.push_back(a_solver[i]);
        v_summary.push_back(a_summary[i]);
    }
    ...
    ud_point * ud_params_st_2_arr = ud_params_st_2.data();

    omp_set_num_threads(num_iter_st2);
#pragma omp parallel for default(shared) private(i)
    for(i = 0; i < num_iter_st2 ;i++){
        //skip the center of second stage(duplicate)
        // Sequential
/*        if(ud_params_st_2[i].C == ud_params_st_1[best_1st_stage].C &&
                ud_params_st_2[i].G == ud_params_st_1[best_1st_stage].G) {
 */
        // Parallel
        if(ud_params_st_2_arr[i].C == ud_params_st_1_arr[best_1st_stage].C &&
                ud_params_st_2_arr[i].G == ud_params_st_1_arr[best_1st_stage].G) {

            // Maybe not needed??
            Solver empty_sv;
            summary empty_summary;
            a_solver[i+num_iter_st1] = empty_sv;
            a_summary[i+num_iter_st1] = empty_summary;
            continue;
        }
 
        summary *current_summary = &a_summary[i+num_iter_st1];
        Solver *sv = &a_solver[i+num_iter_st1];
        svm_model * curr_svm_model;
...
/* 
        ...
        curr_svm_model = sv.train_model(m_train_data_p, v_train_vol_p,
                                        ...
                                        ud_params_st_2[i].C,
                                        ud_params_st_2[i].G);
        sv.predict_validation_data(m_VD_p, m_VD_n, current_summary, i + num_iter_st1);//solver_id);
 */

        // Parallel
        curr_svm_model = sv->train_model(temp_td_p_mats[i], temp_td_vol_p_vecs[i],
                                        temp_td_n_mats[i], temp_td_vol_n_vecs[i], 1,
                                        ud_params_st_2_arr[i].C, ud_params_st_2_arr[i].G);
        sv->predict_validation_data(temp_vd_p_mats[i]
                                  , temp_vd_n_mats[i]
                                  , *current_summary
                                  , i + num_iter_st1);//solver_id);
...
/*
    ...
    {
//        paramsInst->print_summary(current_summary,"[MS][UDSepVal]", level, i, stage);
       paramsInst->print_summary(a_summary[i+num_iter_st1],"[MS][UDSepVal]", level, i, stage);
    }
#endif
*/
...
//        ++solver_id;
...
#if dbl_MS_UDSepVal >= 3
    for(int i=num_iter_st1; i<num_iter_st1+num_iter_st2-1; i++){
        paramsInst->print_summary(a_summary[i],"[MS][UDSepVal]", level, i, stage);
    }
#endif

    // Free remaining copies of the TD and VD Mats and Vecs
    for(i=0; i < num_iter_st2; i++) {
        MatDestroy(&temp_td_p_mats[i]);
        MatDestroy(&temp_td_n_mats[i]);
        MatDestroy(&temp_vd_p_mats[i]);
    ...
    // Also free the memory reserved for the arrays
    PetscFree(temp_td_p_mats);
    PetscFree(temp_td_n_mats);
    PetscFree(temp_vd_p_mats);
    PetscFree(temp_vd_n_mats);
    PetscFree(temp_td_vol_p_vecs);
    PetscFree(temp_td_vol_n_vecs);

    // Add stage 2 solvers and summaries to vectors
    for(i=0; i<num_iter_st2; i++) {
        // Skip any empty summaries
        if(a_summary[i+num_iter_st1].perf.size() == 0)
            continue;
        v_solver.push_back(a_solver[i+num_iter_st1]);
        v_summary.push_back(a_summary[i+num_iter_st1]);
    }
...
//    v_ref_results.push_back(refinement_results);
    ...
    // $PARALLEL related, maybe not needed
//   for(i=0; i<num_iter_st1+num_iter_st2; i++)
//        if(a_summary[i].perf.size() != 0)
//            a_solver[i].free_solver("[MS][UDSepVal] ");
...
//    double train_fraction = 1 - (1 / paramsInst->get_main_num_kf_iter());
    ...
    unsigned int num_iter_st2 = paramsInst->get_ms_second_stage();
    std::vector<Solver> v_solver;
//    Solver a_solver[num_iter_st1+num_iter_st2];		// $PARALLEL
    v_solver.reserve(num_iter_st1 + num_iter_st2);
    unsigned int solver_id=0;
...
//    summary current_summary;
    std::vector<summary> v_summary;
//    summary a_summary[num_iter_st1+num_iter_st2];		// $PARALLEL
    v_summary.reserve(num_iter_st1+num_iter_st2);
    ud_params_st_1 = ud_param_generator(1, inh_params, last_c, last_gamma);

//    ud_point *ud_params_st_1_arr = ud_params_st_1.data();	// $PARALLEL
...
/* 
    ...
    // Duplicate TD Mats and Vecs for separate access by omp threads
    Mat *temp_td_p_mats, *temp_td_n_mats, *temp_vd_p_mats, *temp_vd_n_mats;
    Vec *temp_td_vol_p_vecs, *temp_td_vol_n_vecs;

    // Only need up to num_iter_st1 since the stages are split
    // Can free extra copies before stage 2 to save space
    // NOTE: Only true as long as num_iter_st1 >= num_iter_st2
    PetscMalloc1(num_iter_st1, &temp_td_p_mats);
    PetscMalloc1(num_iter_st1, &temp_td_n_mats);
    PetscMalloc1(num_iter_st1, &temp_vd_p_mats);
    PetscMalloc1(num_iter_st1, &temp_vd_n_mats);
    ...
    for(i=0; i < num_iter_st1; i++) {
        MatDuplicate(p_data, MAT_COPY_VALUES, &temp_td_p_mats[i]);
        MatDuplicate(n_data, MAT_COPY_VALUES, &temp_td_n_mats[i]);
        MatDuplicate(m_VD_p, MAT_COPY_VALUES, &temp_vd_p_mats[i]);
        MatDuplicate(m_VD_n, MAT_COPY_VALUES, &temp_vd_n_mats[i]);

        VecDuplicate(v_vol_p, &temp_td_vol_p_vecs[i]);
        VecCopy(v_vol_p, temp_td_vol_p_vecs[i]);
        VecDuplicate(v_vol_n, &temp_td_vol_n_vecs[i]);
        VecCopy(v_vol_n, temp_td_vol_n_vecs[i]);
    }
    // $PARALLEL end
 */

    // - - - - 1st stage - - - -
//#pragma omp parallel for default(shared) private(i)	// $PARALLEL
    for(unsigned int i =0; i < num_iter_st1;i++){
//    for(i =0; i < num_iter_st1;i++){			// $PARALLEL
...
//        int rank = omp_get_thread_num();	// $Debug purposes $PARALLEL
...
//        cout << "[MS][UDIBSepVal] after push_back solver" << endl;
        sv.predict_validation_data(m_VD_p, m_VD_n, current_summary, i);//solver_id);     // The normal predict method for full matrix is useful rather than index base methods
        // $NOT PARALLEL end
/* */

/*
        // $PARALLEL start
        curr_svm_model = sv->train_model_index_base(temp_td_p_mats[i], temp_td_vol_p_vecs[i], temp_td_n_mats[i], temp_td_vol_n_vecs[i], v_p_index, v_n_index,
                                        iter_train_p_end, iter_train_n_end,true, ud_params_st_1[i].C, ud_params_st_1[i].G);
        // The normal predict method for full matrix is useful rather than index base methods
        sv->predict_validation_data(temp_vd_p_mats[i]
                                    , temp_vd_n_mats[i]
                                    , *current_summary
//                                    , a_summary[i]
...
//        printf("thread rank: %d\n", rank);	// $Debug $PARALLEL
        ...
        v_solver.push_back(sv);			// $NOT PARALLEL
        v_summary.push_back(current_summary);	// $NOT PARALLEL

//        std::swap(a_solver[i], sv);
//        std::swap(a_summary[i], current_summary);
//        a_solver[i] = &sv;
//        a_summary[i] = &current_summary;
...
/*
    ...
    // Put solvers and summaries in STL vectors
    for(i=0; i<num_iter_st1; i++) {
        v_solver.push_back(a_solver[i]);
        v_summary.push_back(a_summary[i]);
    }
...
//    omp_set_num_threads(5);				// $PARALLEL
//#pragma omp parallel for default(shared) private(i)	// $PARALLEL
    for(unsigned int i =0; i < num_iter_st2; i++){
//    for(i =0; i < num_iter_st2; i++){			// $PARALLEL
        //skip the center of second stage(duplicate)
        if(ud_params_st_2[i].C == ud_params_st_1[best_1st_stage].C && ud_params_st_2[i].G == ud_params_st_1[best_1st_stage].G) {
//            Solver empty_sv;
//            summary empty_summary;
//            std::swap(a_solver[i+num_iter_st1], empty_sv);
//            a_solver[i+num_iter_st1] = &empty_sv;
//            a_summary[i+num_iter_st1] = empty_summary;
            continue;
        }
...
/* */
        ...
        curr_svm_model = sv.train_model_index_base(p_data, v_vol_p, n_data, v_vol_n, v_p_index, v_n_index,
                                        iter_train_p_end, iter_train_n_end,true, ud_params_st_2[i].C, ud_params_st_2[i].G);
//        sv.test_predict_index_base(p_data, n_data, v_p_index, v_n_index, iter_train_p_end, iter_train_n_end, current_summary,solver_id);
        sv.predict_validation_data(m_VD_p, m_VD_n, current_summary, i+num_iter_st1);//solver_id);     // The normal predict method for full matrix is useful rather than index base methods
        // $NOT PARALLEL end
/* */
...
//        std::swap(a_solver[i+num_iter_st1], sv);
//        std::swap(a_summary[i+num_iter_st1], current_summary);
//        a_solver[i+num_iter_st1] = &sv;
//        a_summary[i+num_iter_st1] = &current_summary;
...
/* 
    ...
    // Add stage 2 solvers and summaries to STL vectors
    for(i=0; i<num_iter_st2; i++) {
        if(a_summary[i+num_iter_st1].perf.size() == 0)
            continue;
        v_solver.push_back(a_solver[i+num_iter_st1]);
...
//    best_sv.predict_VD_in_output_matrix(m_VD_p, m_VD_n, classifier_id, m_all_predict_VD);  //added 021517-1328
    ...
    // $PARALLEL Maybe not needed?
//    for(i=0; i<num_iter_st1+num_iter_st2; i++)
//        if(a_summary[i].perf.size() != 0)
//            a_solver[i].free_solver("[MS][UDIBSepVal] ");
