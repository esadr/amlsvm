...
solution MainRecursion::main(Mat& p_data, Mat& m_P_p_f, Mat& p_WA, Vec& p_vol,
    ...
    if ((p_num_row <  c_limit) && (n_num_row <  c_limit))  {
        ...
        // Refinement terminated early due to training data size becoming too large
        // Need to skip rest of v-cycle
        if(sol_coarser.terminated_early == true) {
            cout << "[MR][MAIN]Early termination at level:" << level << endl;
            //free the memory
            MatDestroy(&p_data);
            MatDestroy(&n_data);
            MatDestroy(&p_data_c);
            ...
            empty_sol_refine.terminated_early = true;
