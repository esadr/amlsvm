...
//    total_num_point_ = p_num_row_ + n_num_row_;
    ...
    // Moved to constructor 072819 - Korey
//    #pragma omp critical
//    {
    read_parameters();
//    }
...
//    num_points = 1;                         /// only for debug $$
    ...
    // Moved from top of file to accommodate multiple threads
    ...
    for (i=0; i< num_points;i++){
        ...
        x = (struct svm_node *) realloc(x, (ncols) * sizeof(struct svm_node));    //No need for +1 as I add the label in index 0 that add one non-zero value to each row
...
void Solver::predict_validation_data(Mat& m_VD_p,Mat& m_VD_n
    ...
    for (i=0; i< num_points_p;i++){
        ...
        if (predict_label == 1)     //correct
            ...
            fn++;                   //false
            result_summary.v_fn_idx.push_back(i);
        }
    }
    // - - - - - negative class - - - - -@@
    target_label = -1;
    for (i=0; i< num_points_n;i++){
        ...
        if (predict_label == -1)    //correct
            ...
            fp++;                   //false
            result_summary.v_fp_idx.push_back(i);
        }
    }
...
//start of reading the VD_p points
    ...
    for (i=0; i< num_points_p;i++){
        ...
        MatGetRow(m_VD_p,i,&ncols, &cols, &vals);
        //allocate required memory
        x = (struct svm_node *) realloc(x, (ncols) * sizeof(struct svm_node));    //+1 is needed since the label is not in index 0
        for (k=1; k< ncols; k++) {  //zero is the label
            x[k-1].index = cols[k];   //since I add the label in the first column +1 is not required anymore
            x[k-1].value = vals[k];
        }
        x[k-1].index = -1;    //create the end element of each node (-1,0)
        x[k-1].value = 0;
        MatRestoreRow(m_VD_p,i,&ncols, &cols, &vals);
...
//        sol_single_model.n_index.push_back(model_->sv_indices[model_->nSV[0] + i] - 1 - num_point_p); //for normal libsvm
    ...
    std::cout << "[SV][PSSM] feature_size:" << sol_single_model.arr_features_size << std::endl;
