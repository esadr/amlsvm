...
//    exit(1);      //why there is exit here? #strange  #072517-1952 commented
    ...
    int neg_idx_offset = -1;

    for(int i=0; i < num_points; i++){
#if dbl_PR_CPM >= 5
        printf("[PR][CPM] idx: %5d labels => target: %.2f predicted: %.2f\n", i, v_target_labels[i], v_final_predicted_label[i]);
//        std::cout << "[PR][CPM] labels =>  target: " << v_target_labels[i] << " predicted: " << v_final_predicted_label[i] <<std::endl;
        ...
        if(v_target_labels[i] == 1){                            // ++++ positive class ++++
            ...
            if(neg_idx_offset == -1) {
                neg_idx_offset = i;	// Idx where negative class starts
            }
            if (v_final_predicted_label[i] < 0)
                tn++;                   // true negative
            else {
                fp++;                   // false positive
//                printf("Negative class - index: %d; index w/ offset: %d\n", i, i-neg_idx_offset);
                result_summary.v_fp_idx.push_back(i - neg_idx_offset);	// Ensures value added to vector is idx into the separated negative class Mat
