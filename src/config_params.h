...
//    double  ms_validation_part;
    ...
    // Added 6/21/19 - Korey
    bool    rf_aug_nn;
    bool    rf_aug_both_classes;
//    int     rf_aug_nn_num;
    float   rf_aug_drop_threshold;
    bool    rf_aug_drop_from_best;
    float   rf_aug_nn_ratio;
//    int     rf_aug_nn_ratio_metric;
...
//    void set_inputs(string const ds_name, string const ds_version );
    ...
    // Added 6/21/19 - Korey
    bool    get_rf_aug_nn()			const { return (bool) stoi(options_["rf_aug_nn"]); }
    bool    get_rf_aug_both_classes()		const { return (bool) stoi(options_["rf_aug_both_classes"]); }
//    int     get_rf_aug_nn_num()			const { return stoi(options_["rf_aug_nn_num"]); }
    float   get_rf_aug_drop_threshold()		const { return stof(options_["rf_aug_drop_threshold"]); }
    bool    get_rf_aug_drop_from_best()		const { return (bool) stoi(options_["rf_aug_drop_from_best"]); }
    float   get_rf_aug_nn_ratio()		const { return stof(options_["rf_aug_nn_ratio"]); }
//    int     get_rf_aug_nn_ratio_metric()	const { return stoi(options_["rf_aug_nn_ratio_metric"]); }
