...
#define verbose true  // print C, G for the best level
...
//                 "\nms_validation_part: " << get_ms_validation_part()   <<
    ...
    cout << "--- Refinement Paramters ---" <<
		 ...
		 // Added 6/21/19 - Korey
                 "\nrf_aug_nn: "		<< get_rf_aug_nn()			<<
                 "\nrf_aug_both_classes: "	<< get_rf_aug_both_classes()		<<
//                 "\nrf_aug_nn_num: "		<< get_rf_aug_nn_num()				<<
                 "\nrf_aug_drop_threshold: "	<< get_rf_aug_drop_threshold()		<<
                 "\nrf_aug_drop_from_best: "	<< get_rf_aug_drop_from_best()		<<
                 "\nrf_aug_nn_ratio: "		<< get_rf_aug_nn_ratio()		<<
//		 "\nrf_aug_nn_ratio_metric: " 	<< get_rf_aug_nn_ratio_metric()		<<
    ...
    cout << "END PARAMETER CONFIG" << endl;
//    */
}
...
void Config_params::read_classification_training_parameters(pugi::xml_node& root,int argc, char * argv[]){
    ...
    rf_aug_nn			= root.child("rf_aug_nn").attribute("boolVal").as_bool();
    rf_aug_both_classes		= root.child("rf_aug_both_classes").attribute("boolVal").as_bool();
//    rf_aug_nn_num		= root.child("rf_aug_nn_num").attribute("intVal").as_int();
    rf_aug_drop_threshold	= root.child("rf_aug_drop_threshold").attribute("floatVal").as_float();
    rf_aug_drop_from_best	= root.child("rf_aug_drop_from_best").attribute("boolVal").as_bool();
    rf_aug_nn_ratio		= root.child("rf_aug_nn_ratio").attribute("floatVal").as_float();
//    rf_aug_nn_ratio_metric	= root.child("rf_aug_nn_ratio_metric").attribute("intVal").as_int();
...
//    parser_.add_option("--iw", "--inverse_weight")           .dest("inverse_weight")  .set_default(inverse_weight);
    ...
    parser_.add_option("--rf_aug_nn")			     .dest("rf_aug_nn")  .set_default(rf_aug_nn);
    parser_.add_option("--rf_aug_both")			     .dest("rf_aug_both_classes")  .set_default(rf_aug_both_classes);
//    parser_.add_option("--rf_aug_num")			     .dest("rf_aug_nn_num")  .set_default(rf_aug_nn_num);
    parser_.add_option("--rf_aug_t")			     .dest("rf_aug_drop_threshold")  .set_default(rf_aug_drop_threshold);
    parser_.add_option("--rf_aug_frm_best")		     .dest("rf_aug_drop_from_best")  .set_default(rf_aug_drop_from_best);
    parser_.add_option("--rf_aug_r")			     .dest("rf_aug_nn_ratio")  .set_default(rf_aug_nn_ratio);
//    parser_.add_option("--rf_aug_r_m")			     .dest("rf_aug_nn_ratio_metric")  .set_default(rf_aug_nn_ratio_metric);
...
void Config_params::print_final_results() const{
    ...
    for(unsigned int i=0; i< this->all_summary.size(); i++){
        printf("[CP][PFR],it:%d, BestL:%d, AC:%.3f, SN:%.3f, "\
               "SP:%.3f, PPV:%.3f, NPV:%.3f, F1:%.3f, GM:%.3f, Aug:%s",
               ...
               this->all_summary[i].augmented ? "true" : "false");
    #if verbose
